...
func backtracking2(candidates []int, path []int, target int, start int, res *[][]int) {
	if target == 0 {
	...
	for i := start; i < len(candidates); i++ {
		if target-candidates[i] >= 0 {
			...
			// 如果数组中的每一个元素只能使用一次，就是i+1
			backtracking2(candidates, path, target, i+1, res)
			target += candidates[i]
...
func backtracking(candidates []int, path []int, target int, start int, res *[][]int) {
	...
	for i := start; i < len(candidates); i++ {
		if target-candidates[i] >= 0 {
			...
			backtracking(candidates, path, target, i, res)
			target += candidates[i]
			path = path[:len(path)-1]
		}
	}
