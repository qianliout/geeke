...
func maximumGap(nums []int) int {
	...
	for _, n := range nums {
		min, max = utils.Min(min, n), utils.Max(max, n)
	}
	//
	if min == max {
		return 0
	...
	bucketSize := utils.Max(1, (max-min)/(len(nums)-1)) // 桶间距至少为1, 桶里也可能有最大的间距
	// 题目中说了非负
	...
	for _, n := range nums {
		...
		buckets[loc].min = utils.Min(buckets[loc].min, n)
		buckets[loc].max = utils.Max(buckets[loc].max, n)
	}
	// 找值了
	ans := math.MinInt64
	...
	for i := range buckets {
		if buckets[i] != nil {
			if preMax != math.MaxInt64 {
				ans = utils.Max(ans, buckets[i].min-preMax)
			}
			preMax = buckets[i].max
			ans = utils.Max(ans, buckets[i].max-buckets[i].min)
		}
	}
	return ans
}
