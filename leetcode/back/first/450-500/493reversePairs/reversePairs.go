...
// 排序,使用二分查找(超时，因为slice的扩容和复制)
func useBitset(nums []int) int {
	ans := 0
	bitset := make([]int, 0)
	for i := len(nums) - 1; i >= 0; i-- {
		left := utils.FindSmallIdx(&bitset, nums[i])
		ans += left
		utils.FindSmallIdxAndInsert(&bitset, 2*nums[i])
	}
	return ans
}
