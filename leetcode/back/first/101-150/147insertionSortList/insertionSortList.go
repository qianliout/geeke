...
*/
func insertionSortList(head *listnode.ListNode) *listnode.ListNode {
	if head == nil || head.Next == nil {
		return head
	}
	dump := new(listnode.ListNode) // 维护了一个已排序的一个List
	for head != nil {
		curr := head
		head = head.Next
		p := search(dump, curr)
...
// 返回插入节点插入位置的前一个节点
func search(head, node *listnode.ListNode) *listnode.ListNode {
