...
func canCross(stones []int) bool {
	...
	// 怕这种（1，22334343432343)
	// 每一每都跳k+1,如果还到不了最后，那就不能到
	if (1+len(stones))*len(stones)/2 < n {
		return false
	}

	dp[1] = map[int]bool{1: true}
	for i := 1; i < len(stones); i++ {
		for key := range dp[stones[i]] {
			if stones[i]+key-1 <= n {
				dp[stones[i]+key-1][key-1] = true
			}
			if stones[i]+key <= n {
				dp[stones[i]+key][key] = true
			}
			if stones[i]+key+1 <= n {
				dp[stones[i]+key+1][key+1] = true
			}
