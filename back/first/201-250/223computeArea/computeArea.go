...
func compute(point [][]int) int {
	...
	// 计算重叠区间
	if common.Max(point[0][0], point[1][0]) <= common.Min(
		point[2][0], point[3][0]) || common.Min(point[0][0], point[1][0]) >= common.Max(point[2][0], point[3][0]) || common.Max(
		point[0][1], point[1][1]) <= common.Min(
		point[2][1], point[3][1]) || common.Min(point[0][1], point[1][1]) >= common.Max(point[2][1], point[3][1]) {
		return maxArea
	}
