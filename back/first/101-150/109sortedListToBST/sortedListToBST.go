...
func main() {
	...
	head.Next.Next = &listnode.ListNode{Val: 0}
	head.Next.Next.Next = &listnode.ListNode{Val: 5}
	head.Next.Next.Next.Next = &listnode.ListNode{Val: 9}
...
func sortedListToBST(head *listnode.ListNode) *treenode.TreeNode {
	...
	length := -1
	cur := head
	listMap := make(map[int]*listnode.ListNode)
	for cur != nil {
		length++
		listMap[length] = cur
		cur = cur.Next
...
func buildTree(listMap map[int]*listnode.ListNode, start, end int) *treenode.TreeNode {
	if start > end {
		return nil
	}
