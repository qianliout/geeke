...
func largestRectangleArea(heights []int) int {
	...
	// 这种方式,对go语言来说,内存开销大,但是前后加零,确实很巧妙
	...
	stark := make([]int, 0)
	for i := 0; i < n; i++ {
		if len(stark) > 0 && heights[stark[len(stark)-1]] > heights[i] {
			// 出栈,并计最值
			for len(stark) > 0 && heights[stark[len(stark)-1]] > heights[i] {
				...
				if (right-left+1)*heights[pop] > maxArea {
					maxArea = (right - left + 1) * heights[pop]
				}
			}
		}
